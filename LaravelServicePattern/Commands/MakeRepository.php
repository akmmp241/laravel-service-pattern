...
class MakeRepository extends Command
{
    ...
    {
        $name = str_replace(config("service-pattern.repository_interface_suffix"), "", $this->argument("name"));
    ...
    {
        $repositoryInterfaceNamespace = config("service-pattern.repository_namespace") . "\\".$className;
        $repositoryInterfaceName = $className . config("service-pattern.repository_interface_suffix");
        $stubProperties = [
    ...
    {
        $repositoryNamespace = config("service-pattern.repository_namespace") . "\\" . $className;

        $repositoryName = $className . config("service-pattern.repository_suffix");
        $stubProperties = [
    ...
    private function getRepositoryInterfacePath($className)
    {
        return $this->appPath() . "/" .
            ...
            "/$className/$className" . config("service-pattern.repository_interface_suffix") . ".php";
    }
    ...
    {
        $path = $isDefault
            ? "/" . $className . "/$className" . config("service-pattern.repository_suffix").".php"
            : "/Other/$className" .  config("service-pattern.repository_suffix") . ".php";

        return $this->appPath() . "/" .
            config("service-pattern.repository_directory") . $path;
    }
    ...
    {
        $this->ensureDirectoryExists(config("service-pattern.repository_directory"));
        $this->ensureDirectoryExists(config("service-pattern.repository_directory") . "/". $className);
        $this->ensureDirectoryExists(config("service-pattern.repository_directory") . "/" . $className);
    }
}
