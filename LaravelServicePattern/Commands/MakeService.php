...
namespace LaravelServicePattern\Commands;

use Illuminate\Console\Command;
use Illuminate\Support\Str;
use LaravelServicePattern\AssistCommand;
use LaravelServicePattern\CreateFile;
use File;
...
class MakeService extends Command
{
    ...
    {
        $name = str_replace(config("service-pattern.service_interface_suffix"), "", $this->argument("name"));
    ...
    public function createService(string $className)
    {
        ...
        $serviceName = $nameOfService . config("service-pattern.service_suffix");

        $namespace = $this->getNameSpace($className);
        $stubProperties = [
            "{namespace}" => $namespace,
            "{serviceName}" => $serviceName,
            "{serviceInterface}" => $nameOfService. config("service-pattern.service_interface_suffix"),
        ...
        // check folder exist
        $folder = config('service-pattern.service_directory' . "/$serviceName");
    ...
    {
        $nameOfService = $this->getServiceName($className);
        $serviceName = $nameOfService . config("service-pattern.service_interface_suffix");
    ...
    private function getServicePath($className, $servicename)
    {
        return $this->appPath() . "/" .
            ...
            "/$className". "/$servicename" . config("service-pattern.service_suffix") .".php";
    }
    ...
    {
        return config("service-pattern.repository_namespace") . "\Eloquent";
    }
    ...
    {
        return $className . config("service-pattern.repository_interface_suffix");
    }
    ...
    {
        $this->ensureDirectoryExists(config("service-pattern.service_directory"));
        $this->ensureDirectoryExists(config("service-pattern.service_directory"). "/$classname");
    }
    ...
    private function getNameSpace($className):string {
        ...
        if (count($explode) > 1) {
            ...
            return config("service-pattern.service_namespace").$namespace."\\".end($explode);
        } else {
            return config("service-pattern.service_namespace")."\\".$className;
        }
    }
